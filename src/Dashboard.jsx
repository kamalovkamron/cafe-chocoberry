...
const [paymentMethod, setPaymentMethod] = useState('Naqd'); // Asosiy holat: Naqd
  return (
...
  // 1. "Ertaga qoladi" qiymatlarini bazaning 'menu' qismiga 'stock' qilib yozib chiqamiz
  // Buning uchun hozirgi menu'dagi barcha itemlarni aylanib chiqamiz
  const updatePromises = [];
  
  // Menu'dagi barcha mahsulotlarni bazadan o'qiymiz va yangilaymiz
  onValue(ref(db, 'menu'), (snapshot) => {
    const menuData = snapshot.val();
    if (!menuData) return;

    // Hozirgi kunda qancha sotilganini (chiqim) bilish uchun activeShift.orders dan foydalanamiz
    const salesMap = {};
...
    // Har bir mahsulot uchun: stock = (eski_stock + kirim) - chiqim
...
          // Yangi qoldiqni saqlaymiz va kirimni 0 ga tushiramiz (yangi kun uchun)
          updatePromises.push(set(ref(db, `menu/${cat}/${id}`), { 
            ...item, 
            stock: newStock, 
            kirim: 0 // Yangi kun uchun kirimni tozalaymiz
          }));
        });
      }
    });

    // 2. Barcha bazani yangilagandan so'ng, kunni yopamiz
...
        });
    });
  }, { onlyOnce: true }); // Faqat bir marta o'qish uchun
};
...
        <button 
      onClick={() => { window.location.href = '/prixod'; }} 
      style={{ background: 'rgba(52, 211, 153, 0.1)', border: '1px solid #34d399', color: '#34d399', padding: '8px 16px', borderRadius: '10px', fontSize: '14px', fontWeight: '500', cursor: 'pointer' }}
    >
      📥 Prixod
    </button>
      </header>
...
              </div>
              
              <div className="shift-orders-grid" style={{  }}>
...
    }}>
      
      {/* Sarlavha qatori: "1-zakaz" va to'lov usuli */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <h4 style={{ margin: 0, color: '#fff' }}>{order.title}</h4>
        
        {/* TO'LOV USULI SHU YERDA CHIQADI */}
        <span style={{ 
          fontSize: '11px', 
